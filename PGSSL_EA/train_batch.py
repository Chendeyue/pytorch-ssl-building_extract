...
def val_Data(modelname,device,valname,cfg):
    ...
    val_dataset = VOCDataset(valname, cfg.DataSet, 'val', cfg.DataSet.DATA_AUG)
...
def train(cfg,logger):
    ...
    dataset = VOCDataset(cfg.DataSet.TRAIN_NAME, cfg.DataSet, 'train', cfg.DataSet.DATA_AUG)
    ...
    val_dataset = VOCDataset(cfg.DataSet.TEST_NAME, cfg.DataSet, 'val', cfg.DataSet.DATA_AUG)
    ...
    pre_model = smp.Unet(classes=2)
    pre_modelname = 'pretrain/model_0077500_f1_0.661_r0.554_p0.821.pth'
    dict = torch.load(pre_modelname)
    pre_model.load_state_dict(dict['model'])
    pre_model = pre_model.to(device)

    for iteration, sample_batched in enumerate(data_loader, start_iter):
        images, images2 = sample_batched['image'], sample_batched['origin']
        ...
        targets1 = torch.argmax(pre_model(images), dim=1).to(device)
        targets2 = torch.argmax(pre_model(images2), dim=1).to(device)

        mask = creat_mask(cfg.DataSet.DATA_RESCALE,4,0.05)
        b,c,_,_ = images.shape
        # mask_image =  np.tile(mask[np.newaxis,np.newaxis,:,:], (b,c,1,1))
        ...
        p1, p2, z1, z2,result1,result2,result3 = model(x1=images, x2=images2,x3 = mask_image)

        loss1 = criterion1(result1, targets1)
        loss2 = criterion1(result2, targets2)
        loss3 =  2 - (criterion2(p1, z2).mean() + criterion2(p2, z1).mean())
        loss4 = criterion3(result3,images)
        losses = (loss1+loss2)/2+loss3 + loss4
...
if __name__ == '__main__':
    ...
    # trainname_dir = glob.glob('../Data/Satellite dataset Ⅱ (East Asia)/train*')
    trainname_dir = ['../Data/Satellite dataset Ⅱ (East Asia)/all']
    out_dirs = ['model_train_661']

    for trainname,outdir in zip(trainname_dir,out_dirs):
        device = 'cuda:1'
        # if 'model_train#0.2' in outdir:continue
